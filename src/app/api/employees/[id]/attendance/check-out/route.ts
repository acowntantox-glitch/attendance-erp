...
import { checkOut } from "@/domains/attendance/service";
...
export const POST = withApiHandler(async (_requestId, request: Request, ctxParams: RouteParams) => {
  ...
  if (!parsed.success) {
    throw new ValidationError("Invalid check-out data.", parsed.error.flatten());
  }
  const session = await checkOut(ctx, id, parsed.data);
