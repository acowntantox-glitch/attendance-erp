...
import { endBreak } from "@/domains/attendance/service";
...
export const POST = withApiHandler(async (_requestId, request: Request, ctxParams: RouteParams) => {
  ...
  if (!parsed.success) {
    throw new ValidationError("Invalid break-end data.", parsed.error.flatten());
  }
  const event = await endBreak(ctx, id, parsed.data);
