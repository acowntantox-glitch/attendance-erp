import { apiSuccess, withApiHandler } from "@/lib/api/response";
import { ValidationError } from "@/lib/errors";
import { getRequestContext } from "@/lib/auth/request-context";
import { updateWorkScheduleSchema } from "@/validations/workforce";
import { getWorkSchedule, setWorkScheduleActive, updateWorkSchedule } from "@/domains/workforce/service";

type RouteParams = { params: Promise<{ id: string }> };

export const GET = withApiHandler(async (_requestId, _request: Request, ctxParams: RouteParams) => {
  const ctx = await getRequestContext();
  const { id } = await ctxParams.params;
  const schedule = await getWorkSchedule(ctx, id);
  return apiSuccess(schedule);
});

export const PATCH = withApiHandler(async (_requestId, request: Request, ctxParams: RouteParams) => {
  const ctx = await getRequestContext();
  const { id } = await ctxParams.params;
  const body = await request.json().catch(() => null);
  const parsed = updateWorkScheduleSchema.safeParse(body);
  if (!parsed.success) {
    throw new ValidationError("Invalid work schedule data.", parsed.error.flatten());
  }
  const schedule = await updateWorkSchedule(ctx, id, parsed.data);
  return apiSuccess(schedule);
});

export const DELETE = withApiHandler(async (_requestId, _request: Request, ctxParams: RouteParams) => {
  const ctx = await getRequestContext();
  const { id } = await ctxParams.params;
  const schedule = await setWorkScheduleActive(ctx, id, false);
  return apiSuccess(schedule);
});
